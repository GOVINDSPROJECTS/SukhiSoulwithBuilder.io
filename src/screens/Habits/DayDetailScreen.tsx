...
const DayDetailScreen = () => {
  ...
  const updateHabitsForDate = (rawHabits: any[], progressReports: any[], date: string) => {
    ...
    const habitsForDate: Habit[] = rawHabits.map((habit: any) => {
      const report = progressReports.find(
        (r: any) => r.habit_id.toString() === habit.id.toString() && r.tracked_date === date
      );
      return {
  ...
  const fetchHabitsGraph = useCallback(async () => {
    try {
      ...
      const habits: string[] = rawHabits.map((h: any) => h.title || h.habit_name);
      ...
      progressReports.forEach((report: any) => {
        ...
        if (!reports[habitTitle][weekLabel]) reports[habitTitle][weekLabel] = Array(7).fill(0);
  ...
  const renderTodayView = () => {
    const completion = habitCompletionMap[selectedDate] || { completed: 0, total: habits.length || 1 };
    const progress = completion.total > 0 ? completion.completed / completion.total : 0;
...
const styles = StyleSheet.create({
  ...
  tabBar: {
    flexDirection: 'row', alignSelf: 'center', borderRadius: 12, height: 32,
    margin: wp('1%'), marginBottom: hp('1%'), backgroundColor: '#fff', overflow: 'hidden', width: wp('90%'),
  },
  tab: { flex: 1, justifyContent: 'center', alignItems: 'center', borderRadius: 12, height: wp(5) },
  activeTab: { backgroundColor: '#E1F3F9' },
  tabText: { color: '#245C73', fontWeight: '500', fontSize: wp('3.5%') },
  activeTabText: { color: '#245C73' },
  headerTitle: { color: colors.textPrimary, fontWeight: 'bold', marginLeft: wp('2%'), marginBottom: hp('1.5%'), fontSize: wp('9%') },
  sectionTextWrap: { width: wp(82), marginLeft: wp('4%'), marginBottom: hp('1.5%'), marginTop: hp('2%') },
  sectionTitle: { color: colors.muted, fontWeight: '700', fontSize: wp('6%'), marginBottom: 4 },
  sectionDesc: { color: '#666', fontSize: wp('3.5%'), marginTop: 2, maxWidth: wp('70%'), fontWeight: '400' },
  progressBarWrap: { width: wp(90), height: wp(8), marginBottom: hp('2%'), marginTop: hp('2.5%') },
  progressRow: { flexDirection: 'row', justifyContent: 'space-between', marginHorizontal: wp('1%'), marginBottom: wp(1) },
  progressDate: { fontSize: wp('3.2%'), color: '#2D2D2D', fontWeight: '500', minWidth: wp('28%') },
  progressPercent: { fontSize: wp('3.2%'), color: '#2D2D2D', fontWeight: '500', minWidth: wp('10%'), textAlign: 'right' },
  progressBarBg: { height: 8, backgroundColor: '#A0A0A0', borderRadius: 20, overflow: 'hidden' },
